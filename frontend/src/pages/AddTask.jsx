...
const AddTask = ({ taskHook, onTaskAdded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ title: "", deadline: "", estimated_hours: "", course_name: "", reminder_days_before: 2 });
      onTaskAdded();
    } finally {
      setLoading(false);
    }
  ...
  const inputStyle = {
    ...
    border: "3px solid #e0e0e0", fontSize: "15px",
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-3xl font-black" style={{ color: 'var(--dark)' }}>Tambah Tugas ✏️</h2>
        <p className="font-semibold" style={{ color: '#888' }}>Catat sekarang sebelum lupa!</p>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="flex flex-col gap-3">
          {fields.map((field, i) => (
            <div key={field.name} className="relative">
              <div className="absolute left-4 top-1/2 -translate-y-1/2 text-lg">{field.emoji}</div>
              <input
                name={field.name}
                type={field.type}
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full py-4 rounded-2xl font-black text-lg mt-6 transition-all hover:scale-105 active:scale-95 disabled:opacity-50"
          style={{ background: loading ? '#ccc' : 'var(--dark)', color: loading ? '#999' : 'var(--yellow)' }}
        >
